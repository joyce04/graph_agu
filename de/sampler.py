...
from de.normalize import fetch_normalization


# sampling_percent : The percent of the preserve edges. If it equals 1, no sampling is done on adj matrix.
class Sampler:
    """Sampling the input graph data."""

    def __init__(self, train_adj, train_features):

        self.train_adj = train_adj
        self.learning_type = 'transductive'
        self.train_features = torch.FloatTensor(train_features).float()  # dropedge train_features == features
        #
    ...
    def _preprocess_adj(self, normalization, adj, cuda):
        adj_normalizer = fetch_normalization(normalization)  # adjacency normalization after dropping edges to reduce gradient vanish
