...
class FBGCN(nn.Module):
    def __init__(self, n_layer, in_dim, hi_dim, out_dim, dropout):
        ...
        # first layer
        self.fbgcns.append(FBGCN_Layer(in_dim, hi_dim, dropout))
        # inner layers
        for _ in range(n_layer - 2):
            self.fbgcns.append(FBGCN_Layer(hi_dim, hi_dim, dropout))
        # last layer
        self.fbgcns.append(FBGCN_Layer(hi_dim, out_dim, dropout))
        self.dropout = dropout
