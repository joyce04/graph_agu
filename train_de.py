...
from de.util import get_sampler, csr_to_edgelist
...
def train(data, model, optimizer, sampler, args, device):
    (train_adj, train_fea) = sampler.randomedge_sampler(percent=args.de_sampling_percent, normalization=args.de_normalization, cuda=(device == 'cuda'))
    data.x = train_fea
    ...
    with open('./results/nc_de_{}_{}_{}_es_{}.csv'.format(args.gnn, args.epochs, args.dataset, str(args.edge_split)), 'a+') as file:
        ...
        for r in range(10):
            ...
            for epoch in range(args.epochs):
                model.initialize()
                train_loss = train(data, model, optimizer, sampler, args, device)
