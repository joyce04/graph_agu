...
from flag.flag_orig_fb import apply_flag_orig
...
def train_flag_orig(data, model, optimizer, device, params, lsym):
    forward = lambda perturb: model(data.x + perturb, data.train_index, lsym)
    model_forward = (model, forward)

    loss = apply_flag_orig(model_forward, data, optimizer, device, params, lsym)
    return loss

def train_flag_group(data, model, optimizer, device, params, lsym):
    forward = lambda perturb: model(data.x + perturb, data.train_index, lsym)
    model_forward = (model, forward)
        ...
        for r in range(3):
            ...
            for epoch in range(args.epochs):
                ...
                if args.aug_type == 'flag_orig':
                    train_loss = train_flag_orig(data, model, optimizer, device, args, data.lsym)
                ...
                #         gaug.train_predict_edges(data.adj, data.x, data.y, device, 30, args.removal_rate, args.add_rate)
                    ...
                    train_loss = train_flag_group(data, model, optimizer, device, args, data.lsym)
