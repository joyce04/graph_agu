...
from flag.flag_orig_fb import apply_flag_orig, apply_orig_flag_gaug
...
from util.graph import  lap_dinv


def train_flag_orig(data, model, optimizer, device, params, lap, d_inv):
    forward = lambda perturb: model(data.x + perturb, data.train_index, lap, d_inv)
    model_forward = (model, forward)

    loss = apply_flag_orig(model_forward, data, optimizer, device, params, lap, d_inv)
    return loss
        ...
        for r in range(10):
            ...
            for epoch in range(args.epochs):
                ...
                if args.aug_type == 'flag_orig':
                    train_loss = train_flag_orig(data, model, optimizer, device, args, lap, d_inv)
