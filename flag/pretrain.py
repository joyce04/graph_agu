...
def apply_flag_orig_cr_only(model_forward, data, optimizer, device, args):
    ...
    loss = compute_consistency(node_cls, pred_label)
    loss.backward()
    optimizer.step()
...
def apply_orig_flag_gaug_cr_only(model_forward, data, gaug, optimizer, device, args):
